...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.505, iou_thres=0.50, task="detect"):
        """Initialize attributes for the YOLO model."""
        self.task = task
        self.matrix = np.zeros((nc + 1, nc + 1)) if self.task == "detect" else np.zeros((nc, nc))
        self.nc = nc  # number of classes
        self.conf = 0.505 if conf in {None, 0.001} else conf  # apply 0.25 if default val conf is passed
    ...
    def plot(self, normalize=True, save_dir="", names=(), on_plot=None):
        """
        绘制混淆矩阵（已整合删除首行首列、标签倾斜等需求）
        """
        import seaborn  # 延迟导入以加快YOLO主模块加载速度

        # ------------------- 原始矩阵处理 -------------------
        # 归一化计算（原始逻辑）
        array = self.matrix / ((self.matrix.sum(0).reshape(1, -1) + 1e-9) if normalize else 1)
        # 删除第一行和第一列（关键修改点1）
        array = array[1:, 1:]  # 切片删除首行首列（索引0）
        array[array < 0.005] = np.nan  # 过滤过小值不标注

        # ------------------- 图形初始化 -------------------
        fig, ax = plt.subplots(1, 1, figsize=(12, 6), tight_layout=True)  # 适当加宽图形避免标签重叠
        nc = self.nc - 1  # 调整类别数（删除1行1列后）
        nn = len(names)
        labels = (0 < nn < 99) and (nn == self.nc)  # 原标签是否匹配原始类别数

        # ------------------- 调整标签（删除首类别） -------------------
        if labels:
            # 原标签包含背景则删除首类别+背景，否则仅删除首类别
            ticklabels = (list(names)[1:] + ["background"]) if ("background" in (list(names) + ["background"])) else list(names)[1:]
        else:
            ticklabels = "auto"

        # ------------------- 绘制热力图 -------------------
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # 忽略全NaN切片警告
            seaborn.heatmap(
                array,
                ax=ax,
                annot=nc < 30,  # 类别数<30时显示数值
                annot_kws={"size": 10},
                cmap="Blues",
                fmt=".2f" if normalize else ".0f",  # 归一化显示两位小数，否则整数
                square=True,
                vmin=0.0,
                vmax=1.0,  # 关键修改点2：固定热力图区间为0.0~1.0
                xticklabels=ticklabels,
                yticklabels=ticklabels,
            ).set_facecolor((1, 1, 1))  # 背景色设为白色

        # ------------------- 调整x轴标签（倾斜45°，以文本末尾为轴） -------------------
        # 关键修改点3：获取x轴标签并设置旋转和对齐方式
        xtick_labels = ax.get_xticklabels()
        for label in xtick_labels:
            label.set_rotation(30)       # 顺时针旋转45°
            label.set_ha('right')        # 水平右对齐（以文本末尾为基准）
        ...
        fig.savefig(plot_fname, dpi=250, bbox_inches='tight')  # bbox_inches='tight'避免标签被截断
        plt.close(fig)
        if on_plot:
            on_plot(plot_fname)
...
class Metric(SimpleClass):
    ...
    def fitness(self):
        ...
        w = [0.0, 0, 1, 0]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95]
